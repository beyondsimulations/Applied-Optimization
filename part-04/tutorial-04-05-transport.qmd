...
::: {.callout-note}
We can use the `CSV.read` function to load the data from a CSV file into a DataFrame. If we want to access the directory of the current file, we can use the convinient `@__DIR__` macro. For example, to access the file `my_file.csv` in the same directory as the current file, you can use the path `"$(@__DIR__)/my_file.csv"`. If the file is in a subdirectory called `new_data`, you can use the path `"$(@__DIR__)/new_data/my_file.csv"`.
:::
