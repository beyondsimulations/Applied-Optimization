...
This Julia script is an interactive tutorial to introduce you to the transportation problem using JuMP. Here, you will learn how to model the transportation problem and solve it using JuMP and HiGHS. Follow the instructions and confirm your understanding with some small tasks below. 

Make sure to have the JuMP, DataFrames and CSV package as well as the HiGHS solver installed to follow this tutorial.
...
First, we need to load the data and define the fixed parameters. Assume that we are given the following data:
...
## Exercise 1.1 - Define the fixed parameters

Start by defining the fixed scalar parameters that are given. Call them `revenue`, `varCosts`. 
...
#| code-fold: true
# Test your answer
@assert revenue == 11000 && varCosts == 6300
println("Fixed parameters defined successfully!")
```

## Loading the parameters from files

Next, we load the data from the given files. Remember, you can use the `CSV.read` function to load the data from a CSV file into a DataFrame. If we want to access the directory of the current file, we can use the `@__DIR__` macro. 

For example, to access the file `my_file.csv` in the same directory as the current file, you can use the path `"$(@__DIR__)/my_file.csv"`. If the file is in a subdirectory called `new_data`, you can use the path `"$(@__DIR__)/new_data/my_file.csv"`.
...
We can also access the value associated with a specific key in a dictionary by using the key inside square brackets. For example: `available_dict["a_1"]` will return the value associated with the key `"a_1"`.
...
We can also extract the keys and values of a dictionary using the `keys` and `values` functions.
...
::: {.callout-note}
You can also use the `haskey` function to check if a key is in a dictionary. For example, `haskey(travelCosts_dict,("a_1","b_1"))` will return `true` if `("a_1","b_1")` is a key in `travelCosts_dict`, and `false` otherwise.
:::
...
@objective(transport_model, Max, 
    sum((revenue-varCosts-travelCosts_dict[(i,j)])* X[i,j] for i in keys(available_dict), j in keys(requested_dict))
)
```
