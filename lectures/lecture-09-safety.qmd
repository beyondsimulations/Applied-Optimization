...
[Question:]{.highlight} **Have you ever heard of the Hajj?**
:::
...
- Model time preferences with [precomputed penalties]{.highlight}
- Limit **fluctuations** between consecutive periods
:::

## Mina Tent City

![](https://images.beyondsimulations.com/ao/ao_pilgrim-mecca01.png){fig-alt="Aerial view of the Mina tent city near Mecca with thousands of white tents"}

## Mina Tent City

![](https://images.beyondsimulations.com/ao/ao_pilgrim-mecca03.png){fig-alt="Aerial close-up of white tents arranged in dense blocks in Mina"}
...
::: {.columns}
::: {.column width="50%"}
![](https://images.beyondsimulations.com/ao/ao_pilgrim-pedtraf-1.png){width=100% fig-alt="Urban street with individual pedestrians walking in many different directions"}
...
::: {.column width="50%"}
![](https://images.beyondsimulations.com/ao/ao_pilgrim-pedtraf-2.jpg){width=100% fig-alt="Dense crowd of pilgrims moving together in the same direction"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-unidirectional_flow.png){width=30% fig-alt="Diagram of a unidirectional pedestrian flow, everyone moving one way"}
![](https://images.beyondsimulations.com/ao/ao_pilgrim-bidirectional_crossing_flow.png){width=30% fig-alt="Diagram of two pedestrian flows crossing in opposite directions"}
![](https://images.beyondsimulations.com/ao/ao_pilgrim-multidirectional_crossing_flow.png){width=30% fig-alt="Diagram of multiple pedestrian flows crossing in several directions"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-jamarat_flow.png){fig-alt="Map of one-way pilgrim flows around the Jamarat bridge"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-goalconflict.png){fig-alt="Diagram of the goal conflict between time preferences, safety, and simple plans"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-m_a.svg){fig-alt="Schematic of a camp with several feasible one-way paths towards the stoning site"}

## Pilgrim Routes {auto-animate="true" .smaller}

A path may contain one or more bottlenecks, regarded as resources subject to a capacity.

![](https://images.beyondsimulations.com/ao/ao_pilgrim-m_b.svg){fig-alt="Schematic of the paths with bottleneck resources marked along the way"}

## Pilgrim Routes {auto-animate="true" .smaller}

Pilgrims depart from a camp at a time $t$ and pass through the bottleneck later.

![](https://images.beyondsimulations.com/ao/ao_pilgrim-m_c.svg){fig-alt="Schematic of a group departing the camp and reaching a bottleneck in a later period"}

## Pilgrim Routes {auto-animate="true" .smaller}

Our model should assign one of the feasible paths to a camp on all four ritual days.

![](https://images.beyondsimulations.com/ao/ao_pilgrim-m_d.svg){fig-alt="Schematic of one feasible path being assigned to a camp"}

## Pilgrim Routes {auto-animate="true" .smaller}

These bottlenecks should not be overcrowded at any time during the Hajj.

![](https://images.beyondsimulations.com/ao/ao_pilgrim-m_e.svg){fig-alt="Schematic combining camps, assigned paths, bottlenecks, and time periods"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-solap-1.png){fig-alt="Histogram of clustered pilgrim time preferences over the course of a day"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-satisfaction.svg){fig-alt="Different penalty functions around the preferred time, for example linear and quadratic"}
...
- We need to keep the [changes between periods within bounds]{.highlight}
...
Note how we use the subsets $\mathcal{T}_s$ and $\mathcal{P}_s$ [to keep the model small]{.highlight}.
...
[Question:]{.question} **Any idea why?**
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-model_flow.svg){fig-alt="Diagram of groups, paths, and bottlenecks with the time shift between stoning and utilization"}
...
5. Constrain the [relative utilization between periods]{.highlight}
:::
...
[Question:]{.question} **We wanted one path and one time slot per group. Where are these two constraints?**
...
- Each group thus gets exactly [one period and one path]{.highlight}
...
[Question:]{.question} **We promised a capacity limit, but there is no "$\leq$ capacity" constraint. Why does it still hold?**

. . .

- It is [hidden in the variable bound]{.highlight} $U_{r,t} \leq 1$
...
\begin{align*}
& \sum_{p \in \mathcal{P}_r}\sum_{s \in \mathcal{S}_p} n_s \times X_{s,t-a_{p,r},p}  = b_{r,t}\times U_{r,t} && \forall r \in \mathcal{R}, t \in \mathcal{T} \\
& U_{r,t} - U_{r,t-1} \leq \sigma_r && \forall r \in \mathcal{R}, t \in \mathcal{T}: t > 1 \\
& U_{r,t-1} - U_{r,t} \leq \sigma_r && \forall r \in \mathcal{R}, t \in \mathcal{T}: t > 1
\end{align*}
...
\begin{align*}
& X_{s,t,p} \in \{0,1\}  && \forall s \in \mathcal{S}, t \in \mathcal{T}_s, p \in \mathcal{P}_s \\
...
::: {.callout-important}
Capacities are **hard limits**: if demand exceeds them, the model becomes [infeasible]{.highlight} and refuses to produce an unsafe plan. This is a feature, not a bug!
:::

## Capacity Buffers

![](https://images.beyondsimulations.com/ao/ao_pilgrim-capacitybuffers.png){fig-alt="Chart of capacity buffers kept between planned utilization and the full capacity"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-solap-5.png){fig-alt="Comparison of preferred and scheduled departure times after optimization, with peaks flattened"}
...
![](https://images.beyondsimulations.com/ao/ao_pilgrim-media_coverage.png){fig-alt="Collage of newspaper articles covering the Hajj scheduling project"}
...
To learn more about the project and crowd management during the Hajj, take a look at @haase2016improving and the [literature list](../general/literature.qmd) of this course.
