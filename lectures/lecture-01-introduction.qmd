...
## [Split-Order Minimization]{.invert-font} {background-color="#000000" background-image="https://unsplash.com/photos/BNBA1h-NgdY/download?ixid=M3wxMjA3fDB8MXxhbGx8fHx8fHx8fHwxNzYwMzY2OTI4fA&force=true&w=2400" background-size="cover" background-position="center"}
...
12. Passenger Flow Control in Urban Rail
...
![](https://images.beyondsimulations.com/ao/ao_julia-programming-language.png){max-width=400px .r-stretch fig-alt="The logo of the Julia programming language"}
...
[We've just seen the general structure:]{.highlight}
...
[Example]{.highlight}: A truckload from Dresden $i=1$ to Munich $j=2$ costs $c_{1,2}=4640$ Euro. Moreover, it is necessary to **fulfil all customer demands**, as the contract has been signed.

## Graphical Illustration

![](https://images.beyondsimulations.com/ao/ao_transport.png){fig-alt="Illustration of the transport problem: the production sites Dresden and Laupheim are connected by transport routes to the customers in Hamburg, Munich, and Berlin"}
...
[Minimizing the [transport costs]{.highlight} over all truckloads while [meeting the demand]{.highlight} within the [available supply]{.highlight}.]{.fragment}
...
[Minimize]{.highlight} the transport costs over all truckloads while meeting customer demand within the available supply from production sites.
...
[Question:]{.question} Objective value without the supply and demand constraints?

. . .

- The value is [zero]{.highlight}
...
[Question:]{.question} Does anyone have an idea how to write this down?
...
The minimal total cost is [225,460 Euro]{.highlight}.
...
## Description
Unfortunately, the margins on solar panels are low. After the previous contract has been fulfilled, the company will produce the [same number of panels]{.highlight} as before. In addition, all three customers want to order the [same number of truckloads]{.highlight} with solar panels again. The revenue per truckload of panels is 11,000 Euros. The complete production of a truckload of solar panels, including materials, costs 6,300 Euros.
...
[Question:]{.question} What do we need to change here?
...
[Question:]{.question} **One customer will not receive a single panel. Who and why?**
...
- Serving [Hamburg]{.highlight} loses money on both routes!
...
The maximal profit is [129,710 Euro]{.highlight}.
...
[Take a moment to think about what we just built:]{.highlight}
...
![](https://images.beyondsimulations.com/ao/ao_julia-programming-language.png){fig-alt="The logo of the Julia programming language"}
...
![](https://images.beyondsimulations.com/ao/ao_julia2.png){max-width=400px fig-alt="Screenshot of the Julia download page at julialang.org"}
...
![](https://images.beyondsimulations.com/ao/ao_codium_cnl.png){max-width=400px fig-alt="Screenshot of the VS Code editor"}
...
- If not, the IDE likely [cannot find the path]{.highlight} to Julia
