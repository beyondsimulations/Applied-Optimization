...
- $p_{i}$ : production time per unit of flavor $i$ (Parameter)
- $f$ : fixed setup costs, identical for all flavors (Parameter)
- $c_i$ : storage costs per unit of flavor $i$ per period (Parameter)
- $d_{i,t}$ : demand for flavor $i$ in period $t$ (Parameter)
- $w^0_{i}$ : initial inventory of flavor $i$ at the start of period 1, equal to 100 for all flavors (Parameter)
- $q$ : shift length in hours, identical for all periods (Parameter)
- $W_{i,t}$ : quantity of flavor $i$ stored at the end of period $t$ (Variable)
- $X_{i,t}$ : production quantity of flavor $i$ in period $t$ (Variable)
- $Y_{i,t}$ : 1, if flavor $i$ is set up in period $t$, 0 otherwise (Variable)
\newpage
:::
...
$$
X_{i,t} + w^0_{i} -W_{i,t} = d_{i,t} \quad \forall i \in \mathcal{I},t \in \mathcal{T}: t = 1
$$
...
Note that $\sum_{\tau \in \mathcal{T}} d_{i,\tau}$ acts as a Big-M in this linking constraint: any valid upper bound on the production quantity works, but tighter bounds, e.g. $\min(q/p_i, \sum_{\tau \in \mathcal{T}} d_{i,\tau})$, are better for the solver.

$$
\sum_{i \in \mathcal{I}} p_i \times X_{i,t} \leq q \quad \forall t \in \mathcal{T}
$$
...
No, the model is not a nonlinear problem with continuous variables. The objective function and all constraints are linear, and next to the continuous variables $X_{i,t}$ and $W_{i,t}$ the model also contains the binary variables $Y_{i,t}$ — it is a mixed-integer linear program. Thus, the answer is "no" on both counts of the question.
\newpage
:::
...
- $Z_t$ : number of overtime hours in period $t$ (Variable)

**Modified objective function**

$$
\text{min} \quad \sum_{i \in \mathcal{I}}\sum_{t \in \mathcal{T}} (f \times Y_{i,t} + c_i \times W_{i,t}) + \sum_{t \in \mathcal{T}} (s \times q + 1.5 \times s \times Z_t)
$$

Note that $\sum_{t \in \mathcal{T}} s \times q = |\mathcal{T}| \times s \times q$ is a constant: we include it to capture the full labor costs, but it cannot influence the optimal solution.

**New and modified constraints**

$$
\sum_{i \in \mathcal{I}} p_i \times X_{i,t} \leq q + Z_t \quad \forall t \in \mathcal{T}
$$
...
- Key assumptions: Transportation costs are proportional to the quantity shipped (linear), supply at each factory and demand at each customer are known and fixed.

**Split Order Minimization Problem (E-Commerce)**

- Main objective: Minimize the number of split orders to minimize shipping costs.
- Key assumptions: Inventory levels at each warehouse are known, each shipment from a different warehouse counts as a separate split.
...
- Constructs good solutions quickly without exploring the entire solution space
...
Why? For $i = 1,2$ the first branch prints `vectorA[i]` ("Hello", "World"). For $i = 3,4$ the second branch prints `vectorA[i-2]` — the index shift makes "Hello" and "World" appear again. For $i = 5,6$ no condition matches ($i > 6$ and $i == 9$ are both false), so the `else` branch prints "Hi 5" and "Hi 6". For $i = 7,8$ the third branch prints "This is 7" and "This is 8". The `i == 9` branch with the `break` is never reached, as the loop only runs from 1 to 8 — it is dead code.
\newpage
:::
