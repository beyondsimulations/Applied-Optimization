...
First, we load the data from the CSV files.
```{julia}
#| echo: false
#| output: false
using Pkg
...
# YOUR CODE BELOW
distance = Dict()
for i in eachrow(coord)
    for j in eachrow(coord)
        distance[(i.location,j.location)] = sqrt((i.y - j.y)^2 + (i.x - j.x)^2)
    end
end

# Print the first 10 entries of the distance dictionary
println("First 10 entries of the distance dictionary:")
for (k,v) in first(distance,10)
    println(k, " -> ", v)
end
```
```{julia}
...
```{julia}
# YOUR CODE BELOW
@variable(cvrp_model, X[demand.location,demand.location], Bin)       # 1, if arc from node to node exists
```
```{julia}
# Test your code
...
```{julia}
# YOUR CODE BELOW
@objective(cvrp_model, Min, 
    sum(distance[i,j] * X[i,j] for i in demand.location, j in demand.location if i != j)
    )
```
```{julia}
# Test your code
...
Create the constraints that ensure that each location is visited exactly once. **Note that the central location is indexed as `"central"`!** We don't need to consider the central location for the constraints and we have to ensure during summing that we don't include the pairs where `i == j`.

```{julia}
# YOUR CODE BELOW
@constraint(cvrp_model, inflow[j in demand.location; j != "central"], 
    sum(X[i,j] for i in demand.location if i != j) == 1
    )
@constraint(cvrp_model, outflow[i in demand.location; i != "central"], 
    sum(X[i,j] for j in demand.location if i != j) == 1
    )
```
```{julia}
# Test your code
...
Next, we need to restrict the ingoing and outgoing flows to the `central` location. Create the constraints that ensure that the number of vehicles arriving and leaving the central location is equal to the number of vehicles.

```{julia}
# YOUR CODE BELOW
@constraint(cvrp_model, vehicles_in, 
    sum(X[i,"central"] for i in demand.location if i != "central") == vehicles
    )
@constraint(cvrp_model, vehicles_out, 
...
Create the variable `U` and ensure that the capacity usage at each location is between the demand and the capacity. 

```{julia}
# YOUR CODE BELOW
demand_dict = Dict(demand.location .=> demand.demand)
@variable(cvrp_model, demand_dict[i] .<= U[i = demand.location] .<= capacity)
```
```{julia}
# Test your code
demand_dict = Dict(demand.location .=> demand.demand)
...
Next, we need to add the constraints to prevent subtours.
...
@constraint(
    cvrp_model, 
    no_subtour[
        i in demand.location, j in demand.location; 
        j != "central" && i != "central" && 
        i != j
    ], 
    U[i] - U[j] + capacity * X[i,j] <= capacity - demand_dict[j]
    )
```
```{julia}
# Test your code
...
Now, solve the model again and plot the routes.
...
Assume, that FastDelivery wants to know the costs of the optimal solution. Compute the costs of the optimal solution over the timeframe of four weeks with a short calculation based on the results and store the result in a variable called `monthly_costs`. Each driven kilometer is associated with costs of **0.6 EUR per kilometer** (fuel, repairs, and costs for the driver) and each location has to be visited daily from Monday to Friday. Note, that FastDelivery leases its vehicles and it has to pay **450 EUR every four weeks** for each vehicle in use.

```{julia}
# YOUR CODE BELOW
monthly_costs = objective_value(cvrp_model) * 0.6 * 4 * 5 + 450 * vehicles
println("The costs of the optimal solution over the timeframe of four weeks are $(monthly_costs) EUR.")
```
```{julia}
# Test your code
...
println("Costs calculated successfully, great job!")
```
...
```{julia}
#=
We could use a more efficient vehicle that can transport more parcels per drive. Or we could try to reduce the distance between the distribution centers in case we build new ones. Other ideas could be to reduce the number of vehicles, if we don't fully utilize them.
...
```{julia}
# YOUR CODE BELOW
cvrp_model_flex = Model(HiGHS.Optimizer)
set_attribute(cvrp_model_flex, "presolve", "on")
set_attribute(cvrp_model_flex, "time_limit", 60.0)
set_attribute(cvrp_model_flex, "mip_rel_gap", 0.0)

@variable(cvrp_model_flex, X[demand.location,demand.location], Bin)
@variable(cvrp_model_flex, demand_dict[i] .<= U[i = demand.location] .<= capacity)

@objective(cvrp_model_flex, Min, 
    sum(12 * distance[i,j] * X[i,j] for i in demand.location, j in demand.location if i != j)
    + sum(450 * X["central",i] for i in demand.location if i != "central")
    )

@constraint(cvrp_model_flex, inflow[j in demand.location; j != "central"], 
    sum(X[i,j] for i in demand.location if i != j) == 1
    )
@constraint(cvrp_model_flex, outflow[i in demand.location; i != "central"], 
    sum(X[i,j] for j in demand.location if i != j) == 1
    )

@constraint(cvrp_model_flex, vehicles_in, 
    sum(X[i,"central"] for i in demand.location if i != "central") <= vehicles
    )
@constraint(cvrp_model_flex, vehicles_out, 
    sum(X["central",j] for j in demand.location if j != "central") <= vehicles
    )
...
optimize!(cvrp_model_flex)
```

The code below visualizes the results. Note, that you might need to replace the `X` and `U` with your new models variable names, if you have renamed them
```{julia}
