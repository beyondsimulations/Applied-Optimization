...
- $s$ : fixed costs per shift (Parameter)
...
$$
\sum_{i \in \mathcal{I}} p_i \times X_{i,t} \leq q + Z_t \quad \forall i \in \mathcal{I},t \in \mathcal{T}
$$

$$
0 \leq Z_t \leq 1.5 \times q \quad \forall t \in \mathcal{T}
$$
...
- Describe the main objective of the problem (1 point each model)
- Describe two key assumptions of the model (2 points each model)
...
Create a JuMP variable with the following properties: A continuous variable `Production[t,p]` for time periods $t \in 1:\mathcal{T}$ and products $p \in 1:\mathcal{P}$ which must be non-negative and has an upper bound stored in parameter `capacity[t,p]`.
...
```{julia}
@variable(model, 0 <= Production[t in T, p in P] <= capacity[t, p])
```
